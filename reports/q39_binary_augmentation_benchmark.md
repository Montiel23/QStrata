# Q39 — Binary Augmentation Benchmark Report

**Date:** 2026-05-29
**Dry-run:** True
**Epochs:** 4
**Seed:** 45
**Total wall time:** 1.2 min

---

## 1. Objective

Benchmark controlled torchvision-based augmentation strategies on the compact binary classical baseline (q34a_trial_004, 2,250 params, frozen C006-D040 backbone) to determine whether augmentation improves AUROC/F1 after the preprocessing gains from Q38A/Q38C.
...
- `small_affine`: `RandomAffine(degrees=5, translate=(0.03, 0.03), scale=(0.97, 1.03))`
- `mild_brightness_contrast`: `ColorJitter(brightness=0.08, contrast=0.08)`
- `combined_light`: horizontal_flip + small_rotation + mild_brightness_contrast
...
|---|---|---|---|---|---|---|---|---|---|
| no_augmentation | 0.5610 | 0.0000 | -0.1225 | -0.6398 | -0.1629 | -0.6779 | 0.00 | 0.00 | f1_degraded |

## 7. Optimised CLAHE Track (Track B) Results

| Variant | AUROC | F1 | ΔAUROC_base | ΔF1_base | ΔAUROC_Q38C | ΔF1_Q38C | Aug ms | PP ms | Stability |
|---|---|---|---|---|---|---|---|---|---|
| clahe_no_augmentation | 0.5931 | 0.0000 | -0.0904 | -0.6398 | -0.1308 | -0.6779 | 0.00 | 2.43 | f1_degraded |

## 8. Cross-Track Comparison

| Config | AUROC | F1 | ΔAUROC_base | ΔF1_base | Track |
|---|---|---|---|---|---|
| clahe_no_augmentation | 0.5931 | 0.0000 | -0.0904 | -0.6398 | B |
| no_augmentation | 0.5610 | 0.0000 | -0.1225 | -0.6398 | A |

## 9. Best AUROC Configuration

**clahe_no_augmentation**

- AUROC: 0.5931 (Δ-0.0904 vs baseline)
- F1:    0.0000 (Δ-0.6398 vs baseline)
- ΔAUROC vs Q38C best: -0.1308
- Augmentation overhead: 0.00ms
- Preprocessing overhead: 2.43ms
...
- AUROC: 0.5610 (Δ-0.1225 vs baseline)
- F1:    0.0000 (Δ-0.6398 vs baseline)
- ΔAUROC vs Q38C best: -0.1629
- Stability: f1_degraded
...
- AUROC: 0.5931 | F1: 0.0000 | Stability: f1_degraded

## 12. Cost/Performance Tradeoff

| Config | AUROC | Aug overhead (ms) | PP overhead (ms) | Total overhead (ms) |
|---|---|---|---|---|
| clahe_no_augmentation | 0.5931 | 0.00 | 2.43 | 2.43 |
| no_augmentation | 0.5610 | 0.00 | 0.00 | 0.00 |

## 13. Scientific Interpretation

1. **Does light augmentation improve raw baseline performance?** → Best Track A AUROC: no_augmentation (0.5610)
2. **Does light augmentation improve CLAHE performance?** → Best Track B AUROC: clahe_no_augmentation (0.5931)
3. **Does augmentation recover/stabilise F1?** → Track A best F1: 0.0000 | Track B best F1: 0.0000
4. **Does CLAHE + augmentation outperform CLAHE alone?** → Q38C CLAHE-only: AUROC 0.7239 | Best Track B: 0.5931
